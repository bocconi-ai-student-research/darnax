...
class MajorityPooling(Adapter):
    ...
    def __call__(self, x: Array, rng: KeyArray | None = None) -> Array:
        ...
        # unpack patches using the SAME convention as your conv utils:
        # interpret packed axis as (Cin, Kh, Kw) then transpose to (Kh, Kw, Cin)
        patches = patches.reshape(n, ho, wo, cin, kh, kw)
        patches = jnp.transpose(patches, (0, 1, 2, 4, 5, 3))  # (N,Ho,Wo,Kh,Kw,Cin)

        sums = jnp.sum(patches, axis=(-3, -2))  # sum over Kh,Kw -> (N,Ho,Wo,Cin)
