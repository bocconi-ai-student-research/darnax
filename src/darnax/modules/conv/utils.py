...
def conv_backward(
    ...
    -------
    Array
        Kernel gradient of shape (Kh, Kw, Cin_g, Cout_total), normalized by
        1 / sqrt(N × Ho × Wo).
    ...
    n2, ho2, wo2, cpack = patches.shape
    assert cpack == kh * kw * Cin
    assert (n2, ho2, wo2) == (n, ho, wo)
    ...
    scale = 1.0 / jnp.sqrt(jnp.asarray(n * ho * wo, dtype=patches.dtype))
    ...
    patches_g = patches.reshape(n, ho, wo, kh, kw, groups, Cin_g)
    out_g = y.reshape(n, ho, wo, groups, Cout_g)

    accum_g = jnp.einsum("nhwklgc, nhwgo -> klgco", patches_g, out_g)
    accum_g = accum_g * scale

    accum = jnp.transpose(accum_g, (0, 1, 3, 2, 4)).reshape(kh, kw, Cin_g, Cout)
    return accum
...
def conv_backward_with_threshold(
    ...
    -------
    Array
        Gated, variance-normalized kernel gradient of shape (Kh, Kw, Cin_g, Cout_total).
    ...
    patches_g = patches.reshape(n, ho, wo, kh, kw, groups, Cin_g)
    out_g = out_masked.reshape(n, ho, wo, groups, Cout_g)
...
def conv_transpose_backward_with_threshold(
    ...
    -------
    Array
        Kernel gradient of shape (Kh, Kw, Cin_g, Cout_total), variance-normalized.
